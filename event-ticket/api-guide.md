...
This API is hardened according to OWASP guidelines:

- **Rate Limiting (Throttler)**: All endpoints are protected by a global rate limiter. A single IP address can make a maximum of **60 requests per minute**. Exceeding this will result in a `429 Too Many Requests` response.
- **Helmet Headers**: Uses `helmet` to set secure HTTP headers (protects against XSS, Clickjacking, MIME sniffing).
- **Strict CORS**: Cross-Origin Resource Sharing is configured to only allow authorized origins (via `ALLOWED_ORIGINS` env var) and restricts to standard REST methods (`GET, POST, PUT, PATCH, DELETE`).
- **Data Validation & Sanitization**: The global `ValidationPipe` strictly validates payloads and uses `whitelist: true` & `forbidNonWhitelisted: true` to prevent Mass Assignment and NoSQL/SQL Injection payloads.
